...
if curl -s -o /dev/null -w "%{http_code}" --max-time 5 http://app:8080/ 2>/dev/null | grep -q "200"; then
  check "Discovered app responds on port 8080 (the intentional fault — wrong port)" "pass"
else
  check "Try: curl http://app:8080/ — the app is running but on a non-standard port" "fail"
fi

# Check 3: The standard port 80 does NOT work (confirms the fault is reproduced)
http_code_80=$(curl -s -o /dev/null -w "%{http_code}" --max-time 3 http://app/ 2>/dev/null || echo "000")
if [ "$http_code_80" != "200" ]; then
  check "Confirmed: http://app/ (port 80) is not responding — fault reproduced" "pass"
else
  check "http://app/ returned 200 — the fault may have been fixed or is not the port issue" "fail"
fi
